...
*@desc
 */
func TableListAll(mysql *gorm.DB,mysqldb string) []TableList {
	var tableList []TableList
	err3 := mysql.Raw("select table_name from information_schema.tables where table_schema=?", mysqldb).Scan(&tableList).Error
	if err3!=nil{
		log.Fatal(err3)
	}
	return tableList
 ...
 */
func CodeModelList()  {
	mysql, mysqldb := MysqlConn()
	tableList := TableListAll(mysql, mysqldb)
	...
	if SelectType==1{
		tableName := ModeCode(tableList, mysql, mysqldb, fileName)
		...
		for _, table := range tableList {
			OneTable(mysql,mysqldb,table.TableName ,fileName )
...
func OneTable(mysql *gorm.DB,mysqldb,SelectTableName string,fileName *FileNameChange) string  {
	...
	for _, info := range tableInfoList {
		...
		gorm := fmt.Sprintf(`"%scolumn:%s;" form:"%s" json:"%s"  binding:"%s"`,pri,info.COLUMN_NAME,info.COLUMN_NAME,info.COLUMN_NAME,required)
		var sprintf string
